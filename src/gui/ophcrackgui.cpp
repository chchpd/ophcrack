...
#endif
  connect(threadSpinBox, SIGNAL(valueChanged(int)), this, SLOT(threadChanged(int)));
...
void OphcrackGUI::updateConfig(void) {
  ...
  threadSpinBox->setValue(nthreads);
...
void OphcrackGUI::threadChanged(int x) {
  if (x != crack->arg->nthreads) {
    arg->nthreads = x;
    threadLabel->setText(QString("restart required"));
  }
  else
    threadLabel->setText(QString(""));
}
//---------------------------------------------------------------------------
